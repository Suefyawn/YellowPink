...
- **Orders can now be archived, and the 12 old WordPress imports have been.** Archiving (order page → Archive) is for orders that are history rather than real trading records: legacy imports, tests, duplicates. An archived order keeps its data and status but disappears from order lists, the dashboard, Finance, Analytics and the daily reminders; find them under Orders → More… → Archived, and unarchive any time. This replaces the earlier interim step where the 12 stuck WordPress-era orders were closed as delivered: they are now archived as well, so their old revenue no longer inflates any dashboard or finance number.
