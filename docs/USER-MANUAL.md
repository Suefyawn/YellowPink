...
- **A database outage now emails the owner.** If the nightly maintenance run finds every one of its jobs failing, which only happens when the database is unreachable, it emails the owner directly with the subject starting "URGENT" and a link to the database probe. The probe itself (`/api/health/db`) is a public page that says whether the store can read its database, checked every five minutes by the uptime monitor, so the team hears within minutes rather than days.
