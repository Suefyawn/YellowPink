...
| Section | What it's for |
|---|---|
| **Sales & occasions** (`/admin/sales`) | The occasions library: a saved storefront "version" (theme, hero copy, announcement bar, coupon code) for every sale of the year — both Eids, Ramadan, Azadi, Independence-family national days, Women's/Mother's/Teachers' Day, 11.11, Blessed Friday, 12.12, the winter and new-year sales, plus quiet observance looks for Ashura and Eid Milad. Each card has **Turn on now** (live immediately) and **Schedule** (runs itself over this year's dates, then switches off); one **Turn seasonal look off** button restores the year-round yellow & pink. Edit any card's dates and copy inline — lunar occasions need fresh dates each year. Activating an event does **not** create its coupon code; set that up in Coupons first. The Branding page's seasonal card still works for anything ad-hoc. |
