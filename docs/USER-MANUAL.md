...
| **Coupons** | Discount codes — create, edit, set limits and expiry, and turn them on/off. Three types: **Percent %** (a percentage off the items), **Fixed PKR** (a set amount off), and **Free shipping** (waives the delivery charge instead of discounting the items — no value needed). |
...
> **Note on banners.** Both storefront banners — the thin **announcement
> bar** and the richer **promo strip** (label, headline, button, optional
> countdown) — are managed in **Settings → Homepage**. Toggle each on or off
> there; there is no separate campaigns section.
