...
- **A gentle refusal deterrent, sent only after a customer confirms.** COD refusals are where losses come from, so confirmed customers now hear — softly — that refused parcels cost the store the full courier round trip and that such addresses get **flagged** (future COD may need advance payment or re-confirmation; deliberately not "blacklisted", since refused-once addresses have accepted a second attempt). Three placements: a new **"Send thanks + delivery note (WhatsApp)"** button on the order page that appears once an order is marked confirmed (Roman Urdu, same voice as the confirmation ask — the ask itself is unchanged so the note can never discourage a YES); one sentence on the **Track Order** page while a COD parcel is still on its way; and a line added to the **COD FAQ**. Nothing appears at checkout or before confirmation.
