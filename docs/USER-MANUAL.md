...
- **Checkout** — a short, COD-first form: phone, full name, address and city
  ...
  their area. Free delivery is earned on what the customer actually pays for
  the products — the subtotal **after** any coupon or referral discount. If a
  discount takes the order below the zone's threshold, the delivery charge
  applies again (the cart's progress bar, the checkout quote and the final
  charge all count it the same way, so nothing shown ever disagrees with the
  bill). Brand free-shipping deals (like NB Sons over Rs 1,999) and free-
  shipping coupons are unaffected.
...
- **JazzCash / Raast QR** — the customer scans your shop's payment code with
  any bank or wallet app and pays into your JazzCash Business account. It needs
  no gateway account and no merchant credentials, so it works today. The code
  appears at checkout, on the thank-you page and on the order tracking page,
  and the confirmation email links back to it. You confirm the order once the
  money arrives, exactly as with a bank transfer. Set it up in
  **Settings → Payments**.
...
- The customer lands on a **thank-you page** showing the order number and a
  ...
  are shown here; for scan-to-pay orders, the payment code is shown here with
  the order total beside it.
...
| Section | What it's for |
|---|---|
| **Homepage rails** (`/admin/homepage-preview`) | "Today's homepage": the exact product tiles each homepage rail serves today, with a one-sentence reason per product — and now the place to *manage* them. On any tile, **Feature** puts a product into the This Week row's daily rotation (Featured ✓ removes it) and **Pin seller** holds it a fixed Best Sellers slot (max two pins lead the rail); a search box adds any published product from the whole catalogue without leaving the page. A **Featured fill-up** switch decides what happens when fewer than 4 products are flagged: on, trending products top the row up automatically; off, the row shows only your picks. The rails' logic (daily rotation, no product in two rails, demand rails claim first) is unchanged — the buttons only set which products it may use. |
...
| **Segments** | Customer groupings (e.g. high-spenders) for targeting and analysis: seven automatic buckets plus **custom segments** you define with your own criteria (order count, spend, recency, city, account status, tags). Each customer row links to their profile, and guests without an email are identified by their phone number so nobody drops out of the counts. Full breakdown in [Segments in detail](#segments-in-detail). |
...
| **Broadcast** | A one-tap WhatsApp send list covering **every customer who left a phone number** (deduplicated by number, newest order supplies the name) — for sale announcements and store-wide news, where Win-back covers only lapsed buyers. Edit the message template (`{name}` and `{link}` fill in per customer; the link carries campaign tracking for **Analytics → Sources**) and the **campaign key**, then work down the list: **Open in WhatsApp** opens your own WhatsApp with the personalised message pre-typed, you press send. Each send is ticked off in a shared checklist so nobody is messaged twice, whichever device or staff member does the sending. Change the campaign key for the next sale and the checklist starts fresh, while the old campaign's record is kept. |
...
| **Messages** | A threaded inbox for the storefront contact form and inbound email — reply by email straight from the thread, with order history in each conversation header. When a customer’s email carries attachments, the thread now lists their file names and sizes (the files themselves are not stored — ask for a WhatsApp resend if you need one), and a reply that fails to send keeps your typed text in the box instead of losing it. Full breakdown in [Messages in detail](#messages-in-detail). |
...
| Section | What it's for |
|---|---|
| **Sales & occasions** (`/admin/sales`) | **The** seasonal control (Branding just links here now). Occasions run on **autopilot**: any occasion with dates and "Runs itself" ticked turns itself on for its window and off after, every cycle — no arming needed (refresh lunar dates each year; overlapping windows go to the shorter, more specific occasion). **Turn on now** overrides the calendar; **Turn seasonal look off** during a calendar window skips just that occasion's window, the next one still runs. The page opens with a **live look** panel — a miniature of the storefront exactly as shoppers see it right now, and which occasion it came from — then the library: one card per occasion, each a complete pre-stored version (palette, announcement bar + coupon, hero copy **and its own hero image**) shown as a small storefront mock-up. Per card: **Turn on now** (live immediately), **Schedule** (runs itself over its dates — every occasion ships with its next window pre-filled; lunar ones are estimates to confirm by moon sighting), and **Preview** — opens the real storefront wearing that look in a new tab, visible only to you, with a black ribbon to exit or publish. Cards show exact **Live**/**Scheduled** badges. Editing the live occasion updates the storefront automatically. **New occasion** (top right) adds an ad-hoc sale to the library; occasions that aren't live can be deleted from their edit panel (tick the confirmation). Each occasion also carries its **coupon settings** (code, percent / fixed / free delivery, value, minimum order, per-customer and total limits, skip-sale-items): **Turn on now**, **Schedule** and the autopilot calendar create or update that code in Coupons automatically, valid for exactly the occasion's dates, and **Turn seasonal look off** switches it off. Each card shows the code's live status (live now / opens on / expired / switched off, uses so far). A code you created by hand in Coupons is adopted, not duplicated. |
| **Coupons** | Discount codes and automatic discounts — three types (Percent %, Fixed PKR, Free shipping), with limits, expiry and an on/off status pill. Full breakdown in [Coupons in detail](#coupons-in-detail). |
| **Blog** | Editorial posts shown in the storefront "Journal" and at `/blog`. Each post has an **Author** field for the byline (defaults to "Yellow Pink Editorial Team"); naming a real expert on health/beauty posts strengthens their search-engine trust signals. The editorial-team byline links to its own **author page** (`/author/yellow-pink-editorial`) with the team's bio and every article it has written — bylines that aren't in the site's author registry stay plain text (author pages are added by the development team, only for real authors). Health posts get a **Medical reviewer** (see below) assigned automatically from their Health topic when saved; you can also pick or change the doctor per post. The credit adds a *"Medically reviewed by Dr. …"* byline and reviewer schema; the doctor sees every article credited to them on their reviewer dashboard (assignment emails were retired on 1 Sep 2026 — the dashboard is the record). An optional **SEO title** replaces the post title in Google results and link shares only (the article heading on the page is unchanged) — useful when the full headline is too long to display in search results; keep it under ~46 characters, "\| Yellow Pink" is appended automatically. The **body editor** now works like a lightweight WordPress: Write/Preview tabs (the preview renders through the exact same cleanup the live page uses), a toolbar for headings, bold/italic, links, lists, quotes, comparison tables and dividers, and an **Image** button that uploads a photo straight into the post (photos are optimised automatically). A live word count shows the derived reading time — leave the **Read time** field blank and it is worked out from the article for you. |
...
| **Newsletter** | Compose and send campaign emails to a chosen audience: **Subscribers**, **Customers** (every distinct order email), **Both, deduped**, or any **custom segment** from the Segments page — anyone who unsubscribed is excluded from every audience. Compose and send, or **save an edition as a draft** and come back to it — drafts sit above the composer with Open and Delete buttons, and sending a draft moves it into the sent history. Manage the subscriber list directly — add, edit, unsubscribe, or resubscribe people. Campaigns send in the **branded layout** (gradient bar, logo band, footer with unsubscribe), and the body supports rich blocks, each on its own paragraph: `# Big headline` (brand hero band), `[[code:AZADI14]]` (coupon-code box), `[[products:slug-one,slug-two]]` (live product cards with image and price pulled from the catalogue at send time), `[[button:Shop now\|/shop]]` (a large button). A **Preview** button shows the exact email before anything sends. |
...
- **Sales** (am I selling more?) — revenue/orders/AOV cards plus a **returning customer rate** card (share of the window's orders placed by people who had bought before, with change vs the previous window) and an **items per order** card, all with mini-trends where a trend applies; the revenue chart; a **new vs returning customers** chart splitting each period's revenue between first-time and repeat buyers; a **sales by city** table of where the window's revenue actually came from; orders by status with links into the filtered list; and top products. At the foot of the tab, a **Cash on delivery confirmation** card: what share of COD orders get cancelled, what share are confirmed before dispatch, and — the number it exists for — how many customers confirmed by pressing the link in their order email rather than needing someone to chase them on WhatsApp. It counts the link only over orders placed since the link went live, so its rate is never diluted by older orders, and it says plainly when there is not yet enough data to read.
...
- **On-page frustration** (Traffic tab, under the Bing card) — Microsoft Clarity's read on where the site itself is fighting shoppers: **rage clicks** (someone clicking the same spot over and over), **dead clicks** (a click that does nothing), **quickbacks** (open a page, immediately go back), **excessive scrolling** and **script errors**, each shown as a raw count and as **the share of sessions affected**, which is Clarity's own figure and stays comparable when traffic moves. Alongside them: sessions, distinct users, pages per session, **average scroll depth**, **active time** and the **share of sessions on a phone**. The window is the **last 3 days** and cannot be longer: that is all Clarity's export API will return. Any of these rising on a checkout or product page is usually a broken control rather than impatience, so the card links straight through to the session recordings. It needs a one-time API token (clarity.microsoft.com → your project → Settings → Data export); until that is set the card explains how to get one.
...
Every panel that reads PostHog counts **storefront visitors only**. Two kinds of
traffic are left out: staff working in `/admin`, and a crawler that has been
scraping the men's-health and fertility pages since July. The crawler matters
because PostHog itself does not catch it — it drives a real browser, so
PostHog's own bot check scores it as an ordinary person — and it was big enough
on the handful of pages it visits to own their numbers outright. See the
15 September entry in [What's new](#9-whats-new) for what it was doing to the
figures.
...
- **Bulk bar** — tick rows to reveal it: **Bulk edit** (below), *Publish*, *Set draft*, *Archive*, set/clear tag, adjust price by %, or delete. Price and stock cells, and each row's status, are editable inline.
...
The *Pricing & stock* section of the product form also takes a **SKU (Stock Keeping Unit)** and a **Barcode (ISBN, UPC, GTIN, etc.)** on simple (no-variant) products, the same two fields as Shopify's Inventory card. Both are optional: the SKU is your internal product code and shows in the products list under the product name, the barcode is the number printed under the barcode on the retail box. Products with variants keep their SKUs on each variant instead, so the form points you to the Variants section for those; the variant table also takes a per-variant **Barcode** next to the SKU.
...
The Segments page has two parts. The **fixed buckets** (VIP, Loyal, Engaged, New / Recent, At risk, Lapsed, Casual) are computed automatically from each customer's order history; the tabs show live counts and each tab lists that bucket's customers.

#### Custom segments

Above the buckets sits **Custom segments**: groups you define yourself, the same idea as Shopify's customer segments but built with simple pickers instead of a query language.

- **Create a segment** (staff with the *Customers, Edit* permission): give it a name and set any mix of criteria: minimum and maximum orders, minimum and maximum spend in PKR, "ordered within the last N days", "has not ordered in N days", a city, one of the fixed buckets, account status (Any / Guests only / Account holders), and customer tags (a customer matches if they carry any ticked tag). A customer belongs to the segment when they meet **every** criterion you set; a field left empty is simply not a filter.
- **Membership is live.** Nothing is stored per customer: the list and the member count are recomputed from order history every time you look, so customers move in and out of a segment by themselves as they order.
- **View members** shows the segment's customers (email or phone, city, orders, revenue, last order), each linking to their profile, with an **Export CSV** button for ad-platform audiences or a spreadsheet.
- **Use it as a newsletter audience.** Every custom segment appears in the Newsletter composer's *Send to* picker with its mailable count. Members without an email address are skipped, and anyone who unsubscribed is excluded, exactly as with the other audiences.
- **Edit and delete** change only the definition. Deleting a segment never touches the customers in it.
...
**Automatic discounts.** Set **Method** to *Automatic* on the create form and give the discount a customer-facing **title** (for example "Azadi Sale: 14% off everything"). There is no code to share: the discount applies by itself to every qualifying basket, and the shopper sees the title on the cart and checkout discount line with a small "applied automatically" note. Only one discount applies per order, and a code the shopper types always wins over an automatic; when several automatics qualify, the basket gets the biggest one. Scheduling, targeting and product scoping all work exactly the same as for codes, and the status pill toggles it on and off the same way. The small code shown under the title in the list (AUTO-…) is internal and never shown to shoppers.

**Buy X get Y.** Set **Discount type** to *Buy X get Y* on the create form to build offers like "buy 2, get 1 free". You fill in two sections, the same shape Shopify uses. **Customer buys** is the quantity and the products the shopper must add to qualify. **Customer gets** is the quantity discounted, the products the discount applies to, and the value: *Free* or a *Percentage* off. **Maximum uses per order** caps how many times the offer repeats in one basket (default once). It works as a discount code or as an automatic, and the coupons list shows it in plain words, for example "Buy 2, get 1 free". Two behaviours worth knowing. First, the discount always lands on the **cheapest qualifying items** in the basket, so the offer cannot be gamed into discounting the priciest one. Second, when the "buys" and "gets" lists contain the **same products**, one item cannot count as both, so "buy 2 get 1 free" on the same product means every third unit is free: three in the basket gives one free, six gives two (if the per-order cap allows). If the basket is short of the buy quantity, the shopper is told how many more qualifying items to add. A coupon cannot switch between Buy X get Y and the amount-off types after creation; the quantities, products and value stay editable in the edit dialog.
...
You can also add a reviewer manually, set a **default** reviewer, and reorder or hide the board. **Only approve genuine, consenting clinicians.**

**How articles get their reviewer.** When a new post is saved with a **Health topic**, the system assigns a board doctor automatically: it picks from the active doctors whose ticked topics cover that topic, spreading the work between them, and falls back to the **default** reviewer when nobody covers it. Posts with the makeup topic (and posts with no health topic) get no reviewer. You can also pick or change the doctor yourself, on the post's edit form or from the assignment pages described below. The credit takes effect immediately: the article shows the "Medically reviewed by Dr. ..." byline and reviewer schema as soon as the doctor is set.

**The doctor always knows.** Every time a doctor is credited (automatically, by hand, or through a reassignment) they receive a branded email naming the article, with a link to read it and an invitation to reply to hello@yellowpink.pk if anything needs correcting. Their reviewer dashboard lists every article carrying their byline, and each one has a **Flag a concern** button that sends a note straight to your admin notifications (linked to the post) so you can fix, reassign or remove the credit. Take those notes seriously and act on them quickly: the byline is the doctor's name on your content.

**Review workload.** The top of `/admin/reviewers` shows a card per active doctor: how many live articles carry their byline, the topics that route to them (edit these in the doctor's profile form on the same page), and their most recent credits. Each credit has a **Reassign** control: pick another active doctor and the byline moves at once, with the notification email going to the newly credited doctor. The **Article assignments** page still gives you the full per-article view, scored against each doctor's topics, for bulk tidy-ups.
...
Below the 404 table sits **Articles linking to products nobody can buy**. That table is fed by real traffic, so a link only appears once someone has hit it; this one is computed from the catalogue on every page load, so it catches the same breakage on the day it happens rather than weeks later. Each row names the article, the target, why it fails and how many times it appears. A **Draft** target is usually the easy case (publish it, or point the article elsewhere); **No such product** means nothing has that slug at all, which is nearly always a typo in the link.
...
what — pick a page and edit. On **Store profile**, the moment you change
anything a dark **Unsaved changes** bar
...
| **Branding & theme** (`/admin/settings/branding`) | Brand colours (pink, yellow, ink) plus a status card saying what seasonal look is live right now — the seasonal makeover itself is managed in **Marketing → Sales & occasions**. |
...
| **Payments** (`/admin/settings/payments`) | Turn each payment method on or off, manage the bank/wallet accounts shown to customers paying by transfer, and set up the scan-to-pay QR code. |
...
| **Integrations** (`/admin/settings/integrations`) | Live status for every third-party service the store uses — Resend (email), PostHog, Sentry, Upstash, JazzCash, Easypaisa, Search Console, Bing Webmaster Tools, Microsoft Clarity, Meta Pixel, WhatsApp. Each card shows whether its env vars are set and (for analytics services) when data last synced. Secret values are never displayed. This is also where you paste the **Search Console**, **Bing** and **Meta (Facebook/Instagram) domain-verification** codes (the Meta one is required in Meta Business Manager before you can run conversion ads), and the **Microsoft Clarity project ID** for session recordings and heatmaps. Medical reviewers are managed on the **Medical Review Board** (Marketing → Medical Review Board), not here — that's the single place to add doctors, set a default, and assign them per article. Finally, **Connect Google — live data & indexing**: once a Google Cloud OAuth app is set up (one-time; the card walks you through enabling the Search Console + GA4 APIs and the redirect URI to register), click **Connect Google account** to sign in. It auto-links your Search Console property and GA4 property, and a **Submit sitemap to Google** button (re)submits your full sitemap so Google re-crawls every page. *(Note: Google has no API to force-index individual pages — submitting the sitemap is the supported, scalable way to push pages for indexing; Google still decides what and when.)* |
...
- **A database outage now emails the owner within fifteen minutes.** The store checks every fifteen minutes that it can read its database. If it cannot, the owner gets an email with the subject starting "URGENT" and a link to the database probe, repeated at most once every six hours while the problem lasts, and a "Recovered" email once it is back. The nightly maintenance run sends the same alert if every one of its jobs fails. The probe itself (`/api/health/db`) is a public page that says whether the store can read its database.

### 15 September 2026

- **The cookie banner now only appears where it is legally needed, and that switches analytics back on for most of your visitors.** It used to be shown to everyone, and **only 28.2% of people ever answered it** — 2,073 sessions out of 2,887 in 90 days just ignored it. That mattered more than it sounds: Google Analytics, Microsoft Clarity and the Meta Pixel are each switched off until someone answers, so **for 72% of your visitors none of those three ever ran**. The Meta Pixel could not see or re-target most people, and Clarity was only ever recording the minority who tapped a button. Meanwhile the visitors who actually need the prompt are **3.1% of traffic** (87 sessions from Europe, the UK and Switzerland in 90 days, against 2,367 from Pakistan). So the banner is now shown only to visitors in those regions, and everyone else is treated as having agreed. **What this changes for you:** analytics, Clarity recordings and Pixel data should start covering far more of your traffic than before, so expect those numbers to jump — that is coverage catching up, not a real increase in visitors. **What has not changed:** anyone who has already made a choice keeps it, including people who said no, and anyone anywhere can still change their mind at any time on the **Privacy** page, which now shows what is actually switched on rather than a blank form. If we cannot tell where someone is, they get the prompt.

- **Cart recovery was telling shoppers to use a discount code that does not exist.** Both recovery messages, the WhatsApp nudge you send from **Customers → Abandoned** and the last-chance reminder email, ended with *"Use code COMEBACK10 for a discount if you order today"*. **There has never been a coupon called COMEBACK10.** The one actually created for this, back on 14 July, is **COMEBACK15** (15% off over Rs 1,500), and two months later it had been used zero times, because nobody had ever been told about it. So anyone who followed one of those messages typed a code that was refused at checkout — on the exact screen where they had already hesitated once. In the 30 days to today that is **four carts worth Rs 24,049**, every one of them messaged by Tanya, none recovered. **What changed:** the code is now stored in a setting (so you can change it without a deploy), and whatever is set is **checked against your coupon list before any message promises it**. If the code is missing, switched off, expired, used up, gives 0%, or has a minimum order the cart does not reach, the sentence offering it is dropped from that message and the rest goes out unchanged — a nudge with no discount still works, one with a broken discount is worse than none. A yellow warning appears at the top of the Abandoned page whenever that happens, naming the reason, so it cannot quietly rot again. **Also:** AZADI14 expired on 15 August but was still switched on, which is how the next stale code would have got promised; it is now off.

- **Why the abandoned-cart alert reaches you hours late.** The code claimed the staff alert fires about an hour after a shopper goes quiet. Measured against the four carts above, it actually took **3, 4, 15.8 and 16.6 hours**. The cause is not a bug: every scheduled job runs inside one daily run at 2pm Pakistan time, because the free Vercel plan only allows a few scheduled entries. The comment now says so honestly. Moving off Vercel to your own server (already on your list) is what lets these split back onto their own schedules and run through the day.

- **Hair Care is now merchandised like a real category, with three new shop pages.** The shelf went from 5 products to 14 in two days and nothing downstream had caught up: there was one hair page, `/collection/hair-care`, whose description still promised "oils, masks and growth treatments" and never mentioned a shampoo or a conditioner. **Three new collections**, sized against what people actually search for in Pakistan rather than guessed: **Shampoo & Conditioner** (7 products, roughly 20,000 searches a month across the shampoo and conditioner terms), **Hair Growth & Hair Fall** (7 products, roughly 14,000), and **Anti-Dandruff** (3 products, roughly 8,000). Each has its own buying guide and four questions answered underneath, which is what gets these pages ranked rather than just listed. Three more were deliberately **not** built for now, because the shelf is too thin to justify a page: hair serum (one product), hair mask (one product) and argan oil (three, and the main search term is hard to rank for). **Also fixed:** the Hair Care description on the category page and in the shop menu, still listing the old five products; five hair products with no subcategory, so they could not be filtered; and the keratin guide's shop button, which said "Shop Set and Touch Keratin Repair Shampoo" while actually going to the argan hair mask, left over from when that line was unpublished. All eight hair guides now link through to whichever of the new pages answers them, so the guides feed the shop pages instead of dead-ending on one product. **On the home page:** the CeraVe anti-dandruff shampoo and the OGX argan oil shampoo and conditioner are now marked **Featured**, so hair appears in the "This week" row for the first time. That row shows four tiles drawn from all seven featured products on a daily rotation, so the three hair products cycle through rather than sitting behind the skincare ones. None of them is marked **Best Seller**, and that is deliberate: they have sold nothing yet, and that badge is a claim made to the shopper rather than a slot on a shelf. The homepage health card will list them under "no shopper activity recently", which is simply true of anything newly featured and not a fault.

- **Two guides that rank for stock we do not carry now point somewhere you can buy.** `/blog/best-facial-kit-in-pakistan` and `/blog/bb-cream-guide-price-pakistan` were the two worst-converting articles with real readers behind them: 8.9% and 3.6% of visitors reached a product page, against 20–26% for the brand guides. Neither had a broken link and both are good writing. The problem was that each ranks for a category the shop deliberately does not stock — the only BB creams and facial kits in the catalogue are the Golden Pearl drafts you keep unpublished — so each answered the reader's question in full and then left them to buy elsewhere. **What changed:** in the facial-kit guide, the price table people actually shop from had **five of its eight rows as plain text**, so most of it could not be clicked; all eight are now links. It also had no closing button at all, and the sentence carrying its whole argument (put the money into a good cleanser, a mild exfoliant, a moisturiser and a sunscreen) linked to nothing, so that sentence is now four links. Both guides' pick boxes used to open by saying what the shop does not sell; they now open with what you get and keep the caveat lower down. The facial picks also gain the Beauty of Joseon wash-off mask above the Garnier sheet mask, because a 150ml tub covers a run of sittings where a sheet covers one. **What did not change:** every honest line about the competing kits and creams stays, including their prices and the fact that ours cost more up front. That honesty is why these guides rank, and no price claim was added that the catalogue does not back up.

- **Analytics no longer counts a crawler as a customer.** A scraper has been working through the store's men's-health and fertility pages since 1 July, and every panel that reads PostHog was counting it as visitors: **421 sessions over 90 days**. It was invisible because it drives a real browser rather than a script, so PostHog's own bot detection scores it as ordinary human traffic. It gives itself away instead by behaving like nothing else on the site: exactly one page per visit and never a second, a different IP address every time, arriving in bursts, and coming from the United States, the UK and Japan when the store is Pakistan and 89% phones. It only ever touches **13 pages**, all of them around Repro-M, X-Fit, Fol Chew and the testosterone and fertility articles, which reads as a competitor keeping an eye on those particular products. **What it had been doing to the numbers:** those 13 pages get little real traffic, so the crawler owned their figures. `/blog/trimo-m-...` looked like 173 visitors who never once clicked through to a product, when its real readership for the whole quarter was **one person**; `/blog/best-prenatal-vitamins-...` had 24 of its 80 visits faked, which pushed a genuine 10.7% click-through down to a reported 7.5%. It was also feeding product views into the popularity scores that order products on the storefront, so four supplements were ranked partly on a robot's attention. All of it is filtered out from today, history included, so the traffic, funnel, landing-page and demand panels now show people. **What this changes for you:** visitor counts on those pages will drop, and that drop is the numbers getting honest, not traffic being lost.

- **Broken links now warns you before a customer finds the broken link.** Until today the **Broken links** screen only listed URLs that had already returned a 404 to a real visitor, which means the store found out by losing that visit. There is now a second block under it, **Articles linking to products nobody can buy**, checked against the catalogue rather than against traffic. The moment a product is drafted or archived, every article that recommends it shows up here, with the article, the product, why it fails (draft, archived, or a slug that matches nothing, usually a typo) and how many times it appears in that article. **Why this was needed:** archiving the Hello Hair brand on 14 September silently broke five product links, two of them on `/blog/best-shampoo-in-pakistan`, which turns out to be the single biggest entry point to the whole store. A full scan then found **23 dead product links across 21 articles**, some months old. All 23 have been repointed at published products, and the claims around them corrected where a replacement's price or ingredients made the old sentence untrue.

- **Eight imported shampoos and conditioners are live, and the quiz can answer a dandruff question again.** CeraVe anti-dandruff shampoo and conditioner, La Roche-Posay Kerium DS, and five OGX lines (Argan Oil shampoo and conditioner, Biotin & Collagen shampoo and conditioner, Tea Tree Mint shampoo). **Hair Care goes from 5 published products to 13.** The reason for doing this now rather than later: `/blog/best-shampoo-in-pakistan` turned out to be the single biggest entry point to the whole store, 216 visitors in 30 days, ahead of the home page and every product page. The store had **zero published shampoos**, so the largest stream of people arriving were asking which shampoo to buy and finding nothing to buy. The **Routine Finder** also gets its **Dandruff & flaky scalp** option back, which had been deliberately withheld while there was nothing to recommend; an anti-dandruff shampoo is filed under "treat the scalp" rather than "look after the lengths", since that is what it is for. **Two things still need you:** the prices are estimates extrapolated from your existing pricing for each brand, not from what you actually pay, and these products are set to never show as out of stock, so an order can arrive for something you do not hold. Both are a single edit on the product page. The Olaplex and The Ordinary hair serum drafts stay unpublished, as do the Set and Touch shampoos, which are the cheap local lines you decided against.

- **The Clarity card was still wrong, and now it is right.** Yesterday's fix stopped it inventing numbers but left it showing a dash for every frustration signal, which was honest and useless. With the API token live, the first refresh stored what Clarity actually sends, and the field names could finally be read off real data instead of guessed: the counts arrive in a field called `subTotal`, and two traffic figures had been quietly reading zero because of a misspelling and a capital letter. **The first real reading: 27.8% of sessions are quickbacks** — someone opening a page and immediately going back — against 7.4% dead clicks and no rage clicks or script errors at all. The card also now shows the share of sessions each signal affects, average scroll depth, active time, and the phone-versus-desktop split, all of which were arriving on the same API call and being thrown away. The tests behind it were rewritten too: the old ones were written against an invented example of the response, so they confirmed the code's own assumptions and could never have caught this.

- **Cash-on-delivery confirmation is now measurable, on Analytics → Sales.** The confirm link shipped on 14 September with no way to tell whether it was working, because a customer pressing the link and a staff member recording a WhatsApp reply both set the same field. Staff confirmations now leave a timeline entry of their own (visible on the order, where they always should have been), so the two can be told apart. The new card shows the cancellation rate, the share of orders confirmed before dispatch, and how many of those confirmations the customer made themselves. **It currently reads "nothing to read yet", which is correct:** no cash-on-delivery order has been placed since the link went live, so it has not had a chance to be pressed. The card also warns you about its own most tempting number: confirmed orders cancel at 17% and unconfirmed ones at 71%, but that gap is partly circular, since staff cancel the orders they could not reach and those are the same orders nobody confirmed.

- **A step-by-step guide to putting the store on its own server.** `docs/SERVER-SETUP.md` covers what the self-hosting notes assumed you already knew: which provider to rent a machine from and what it costs, how to make an SSH key on Windows or Mac, and how to log in. Then a single command does the rest — `scripts/provision.sh` installs everything, gets the HTTPS certificate automatically, sets up the three scheduled jobs and starts the store, stopping partway to have you paste in the settings it cannot know. There is a `scripts/deploy.sh` for updates afterwards, which builds before it restarts so a bad change leaves the previous version running. The guide has you test the new server over the real domain, on your own computer only, before touching DNS, and it does not let you cancel Vercel until a scheduled job has run successfully on the new box.

- **Eight more brand pages got a buyer's guide, and that is the end of the batch work.** St. Ives, Yellow Pink, The Ordinary, COSRX, Charlotte Tilbury, Clearblue, Celimax and Mixsoon. **32 of 59 brand pages now have a guide, up from 14 this morning.** The remaining 27 were deliberately left alone: every one of them has a single published product, and a buyer's guide for one product is a product page with extra steps. The floor used here is two published products, which is the point where the page can answer a real question, namely which of these do I need. **OGX was skipped on purpose** even though it qualifies on demand: it has one published product today and five hair products waiting in Draft, so its guide should be written once that shelf is real rather than rewritten a week later. The Yellow Pink page is worth a read yourself: it says plainly that a combo is a set of products we already stock bundled at a lower price, not something we manufacture.
...
- **The catalogue is imported-only now, as a written rule.** Cheap local lines are not worth stocking: the margin is too thin for the shelf space and the fulfilment effort. This is now recorded in the project's working rules, so a gap in a collection or the hair quiz will not get quietly filled with PKR 150 stock. It means some obvious gaps stay open on purpose. The hair quiz still has no dandruff question, for example, because the only anti-dandruff products in the catalogue were Hello Hair and those are archived. **The cheapest way to close those gaps is imported stock from a supplier you already use:** La Roche-Posay and CeraVe both make anti-dandruff and scalp lines and you already sell both brands, The Ordinary makes a hair-density serum and you already sell three of their products, and OGX (whose Argan mask you already sell) has the matching shampoo and conditioner that would complete the quiz's cleanse step.

- **Six new collection pages, built on product tags.** **Salicylic Acid & BHA**, **Niacinamide**, **Hyaluronic Acid**, **Collagen**, **Vitamin C** and **Foundation** are live, each with a buyer's guide and four FAQs. Together they target roughly 47,000 searches a month in Pakistan (salicylic acid alone is 14,800). Batches 1 and 2 covered product *formats* (sunscreens, cleansers, toners); this batch covers what people type when they know the **ingredient** but not the product. **What changed mechanically:** these use the **Tags** box on the product page rather than a name rule. The collection rules could only read a product's name, and ingredients live in the description, so only 3 products have "niacinamide" in the name while 13 are built around it. Tagging fixes that, and it means **you can add a product to one of these collections just by adding the tag on its product page** — no developer needed. The same tags also now appear as ingredient filters on the shop page. Still waiting on stock rather than demand: **concealer** (6,600 searches a month, only 4 real concealers), **face masks** (4,400, only 4), and **multivitamins** (8,100, only 4 once fertility packs are set aside). Those three are ready to build the moment those shelves are deeper.
...
- **Clarity's rage clicks and dead clicks now show up in the admin.** Clarity has been recording sessions since 4 September, but the numbers only existed inside Clarity's own dashboard, so nobody saw them next to Google and Bing. **Analytics → Traffic** now has an **On-page frustration** card: rage clicks, dead clicks, quickbacks, excessive scrolling and script errors, as counts and as a rate per 1,000 sessions, with a link through to the recordings. These are the closest thing the store has to a direct answer for *where does checkout lose people* — a dead click on the checkout button looks identical to "shoppers just didn't buy" in every other report. The window is the last 3 days because that is the most Clarity's export will give, and it says so on the card. It needs a one-time API token from Clarity (Settings → Data export → Generate new API token, then `CLARITY_API_TOKEN` in the server settings); until that is set the card tells you how to get it rather than showing empty numbers.
...
- **Customers can pay by scanning your shop's QR code.** A new **JazzCash / Raast QR** option at checkout shows your JazzCash Business payment code. The customer scans it with any bank or wallet app (JazzCash, Easypaisa, SadaPay, NayaPay, a normal bank app), pays, and sends you the receipt; you confirm and ship, exactly as with a bank transfer. It needs no gateway account and no merchant credentials. Set it up in **Settings → Payments**: open your QR in the JazzCash Business app, share it to any QR reader to copy the text it contains, and paste that in. The page checks the code the moment you save (a half-copied one is refused rather than quietly failing at checkout), shows you the name customers will see on their payment screen, and draws a preview to scan yourself. The code appears at checkout, on the thank-you page and on the order tracking page, and the confirmation email links back to it, so a customer who closes the tab can still pay. There is one option worth knowing about: **Put each order's amount on the code** writes the exact total and the order number into the code, so the customer's app fills both in and you can tell which order a payment belongs to. Test it with one cheap order before relying on it: if your banking app reads the code with the amount already filled in, leave it on; if not, untick it and customers type the amount themselves, which is how it works by default.
...
- **Microsoft Clarity records how shoppers actually use the site.** Paste the project ID into **Settings → Integrations** and Clarity starts collecting session recordings, heatmaps and rage-click / dead-click reports for the storefront. It only loads for visitors who accept analytics cookies (the same rule as Google Analytics), and it stops recording the moment a staff member opens the admin or the reviewer portal, so order and customer screens are never captured. Watch a few recordings of shoppers who added to cart and left: that is the fastest way to see where checkout loses people.
- **Dated price headings keep themselves current.** Guides that show live prices can now write `[[month]]` (or `[[year]]`) in the copy, and the page renders the real month in Pakistan time, the same way `[[price:slug]]` already renders a live price. The CeraVe guide's "CeraVe Prices in Pakistan" heading used it first: it had been hand-typed, so it claimed a month that would quietly go stale. Headings that state when prices were *checked against other retailers* deliberately keep their fixed month, because that is a fact about the check, not about today.
- **An occasion now publishes its coupon with its look.** Every card on **Marketing → Sales & occasions** has coupon settings (code, percent / fixed / free delivery, value, minimum order, per-customer and total limits, skip-sale-items; the value is pre-filled from the bar message, e.g. "up to 15% off" → 15). **Turn on now**, **Schedule** and the autopilot calendar create or update the code in Coupons, valid for exactly the occasion's dates, so the bar never advertises a code checkout rejects. **Turn seasonal look off** switches the code off; a code you created by hand is adopted rather than duplicated. Each card shows the code's live status and how many times it has been used.
- **Bing Webmaster Tools joins Search Console.** A new **Bing search** card on **Analytics → Traffic** shows Bing / DuckDuckGo / Yahoo / Copilot clicks and impressions (with the prior-28-day comparison), top queries and pages, crawl health and the URL-submission quota. It needs a one-time API key from Bing Webmaster Tools (see Settings → Integrations, which now also has a Bing verification field). Once the key is set, every new product or post is also pushed through Bing's own submission API next to IndexNow.
...
- **Four checkout rules that silently stopped working on 1 September are back.** A change that day rebuilt the order-placing routine and lost four earlier additions. Since then: a product you hold that had run to zero but *keeps selling* was refused at checkout as out of stock; any coupon limited to certain products or excluding sale items, and any Buy X Get Y offer, failed at checkout with a mismatch error; and a *Scheduled* coupon could be used before its start date. All four behave as documented again, and the automated migration check that catches this now runs on every change.
...
- **A coupon no longer keeps free delivery it un-earned.** Free delivery is now judged on the subtotal *after* the discount: an order that only cleared the free-delivery threshold before a coupon was applied pays the normal delivery charge again. The cart progress bar, mini-cart, checkout quote and the server's final charge all count it the same way. Brand deals (NB Sons ≥ Rs 1,999) and free-shipping coupons behave as before.

- **The occasions calendar now flies itself.** Every occasion with dates runs automatically over its window each cycle ("Runs itself" tickbox per occasion, on by default) — the year's sales need no arming at all. Manual "Turn on now" always overrides; "Turn seasonal look off" during a calendar window skips only that window. Two looks changed on owner request: **Blessed Friday** is now deep teal and rich gold, and **Ashura** wears a new subdued black **Quiet / Mourning** theme.
- **Sales & occasions became the one seasonal control, with previews.** Later the same day the page was rebuilt Shopify-Themes-style: a **live look** panel shows a miniature of the storefront as shoppers see it right now, every occasion card renders a mock-up of its stored look, and a **Preview** button opens the real storefront wearing that occasion — visible only to you (a black ribbon at the bottom exits the preview or jumps back to publish). Every occasion now carries its **own hero image** (19 unique, on-palette photographs) and its **next window pre-filled** (2027 dates; lunar occasions marked as estimates to confirm by moon sighting), so the whole year is genuinely one click. Live/Scheduled badges are exact, editing the live occasion updates the storefront automatically, and **New occasion** adds an ad-hoc sale. The Branding page's duplicate seasonal form was retired — it now shows the current status and links here.
- **Every sale of the year is now one click.** The new **Admin → Marketing → Sales & occasions** page holds a saved storefront version for 20 occasions — both Eids, Ramadan, Azadi, the national days, Women's/Mother's/Teachers' Day, 11.11, Blessed Friday, 12.12, winter and new-year sales, and quiet observance looks for Ashura and Eid Milad — each with its theme, hero copy, announcement bar and coupon code. **Turn on now** puts it live, **Schedule** runs it over its dates and switches it off after; dates and copy are editable per card (lunar occasions need new dates each year). Three new looks joined the theme set: Ramadan (midnight blue and gold), Women's Day (plum), and Blessed Friday (ink black with the brand yellow).

- **You control the homepage rows now.** **Admin → Catalogue → Homepage rails** (the old Homepage preview, now in the sidebar) lets you feature/un-feature products in the This Week row, pin/unpin Best Sellers, and search any product straight into a rail — plus a switch that stops the This Week row from topping itself up with trending products when you'd rather it showed only your picks.

- **The blog editor grew up.** Write/Preview tabs (the preview uses the exact same cleanup as the live page), a formatting toolbar, photos *inside* the post body (uploaded and optimised automatically), a live word count, and an automatic reading time when the Read time field is left blank.

- **Image uploads optimise themselves.** Every image uploaded anywhere in the admin (products, blog heroes and bodies, branding) is now automatically straightened, sized to the right dimensions for its spot, and compressed to a fast web format — without altering the picture itself. Files up to 25 MB are accepted, so raw phone photos are fine to upload directly.

- **Cash now shows your total position.** Below the till figures, the Cash page values the tracked stock you own — units on hand, at cost, at retail — and adds **Total position** (cash in hand + stock at cost), with a note when stocked products are missing a cost price.

- **Doctors are no longer emailed about assignments.** Crediting a Review Board doctor on an article (automatic, manual, or reassignment) no longer sends them an email; their reviewer dashboard remains the complete record of every article credited to them. All 42 older posts that had slipped through without a doctor were also backfilled: every health post now carries a credited reviewer (makeup and hair posts stay reviewer-free by design).

- **No more duplicate emails.** One customer received the review thank-you coupon three times (they had submitted the same review three times, and each approval minted a coupon): reviews are now limited to one per shopper per product per 30 days, the thank-you reward is once per product, and the Shipped/Delivered/Cancelled emails can no longer double-send when a staff status change and the courier sync land on the same transition.

- **Small fixes:** the dashboard's Refresh analytics button now retries when PostHog has a slow moment instead of failing the whole refresh; inbound-email attachments are listed in Messages; 11 new beauty products (Beauty of Joseon, COSRX, La Roche-Posay, Celimax, Medicube, Pixi, TFIT — with the TFIT concealer's six shades) and 16 Nutrifactor supplements were added as drafts ready to publish.

### 15 August 2026

- **Medical review runs itself now.** Saving a health article assigns a Review Board doctor automatically: the post's Health topic is matched to the doctors who cover it (work spread between them, with the default reviewer as fallback; makeup posts get no reviewer), and the "Medically reviewed by" byline goes live with the article. The credited doctor is emailed every time (auto, manual or reassignment) with a link to the article and a way to request corrections, and their dashboard gained a **Flag a concern** button that drops a note into your admin notifications. The Medical reviewers page now opens with a **Review workload** view: credited-article counts and topics per doctor, plus their latest credits with a one-click **Reassign** that moves the byline and notifies the new doctor. Details in [Medical reviewers in detail](#medical-reviewers-in-detail).
...
- **Custom customer segments.** The Segments page now lets you define your own segments with simple criteria pickers (order count, spend in PKR, ordered or not ordered within N days, city, fixed bucket, account status, customer tags), the Shopify pattern without the query language. Membership and counts are computed live from order history, each segment's member list can be viewed and exported as CSV, and every segment appears as an audience in the Newsletter composer's *Send to* picker (unsubscribes still excluded, members without an email skipped). Details in [Segments in detail](#segments-in-detail).

- **Buy X get Y offers.** The coupon create form gained a **Discount type** choice: the existing amount-off maths or *Buy X get Y*, built Shopify-style with a **Customer buys** section (quantity + products) and a **Customer gets** section (quantity + products + *Free* or a percentage off, plus a per-order cap). Works as a typed code or an automatic, the cheapest qualifying items receive the discount, and using the same products in both sections makes the classic every-3rd-free offer. Details in [Coupons in detail](#coupons-in-detail).
...
- **A WhatsApp Broadcast page reaches every customer, one tap each.** **Admin → Broadcast** lists every customer who has left a phone number (one row per number — no double-sends to repeat buyers), each with an **Open in WhatsApp** button that opens your own WhatsApp with the personalised message pre-typed; you just press send. The message template and the **campaign key** are editable on the page (`{name}` and `{link}` fill per customer, the link carries campaign tracking into Analytics → Sources), sends are ticked off in a checklist shared across devices and staff, and changing the campaign key starts a fresh checklist for the next sale. It ships pre-loaded with the **Azadi sale** message, ready to work through.

- **Campaign emails can now reach customers, not just subscribers.** The Newsletter composer gained a **Send to** picker: Subscribers, Customers (every distinct email that has ever placed an order), or Both deduplicated. Anyone who unsubscribed is excluded from every audience, and the confirmation dialog always states exactly how many people are about to receive the email. Nothing sends until you press Send.

- **Variants are edited in one grid, like Shopify.** The Variants section is now a single table: every variant's SKU, price, compare-at price, stock and status (Active/Hidden) are editable in place, and one **Save all changes** writes only the rows you actually touched. Stock edits land in Movement history like every other stock change, and the stock column reads "not counted" for vendor-held products. Image, options and sort order live behind each row's **Details** button; Delete sits on the row. A mistakenly created attribute can now also be deleted — pick it in the add-value bar and press **Delete attribute**, which only works while nothing anywhere uses it, so a live product's options can never be stripped by accident.

- **Product options now work the Shopify way.** The Variants section opens with an **options card**: each axis the product varies on (Shade, Size…) with its values as chips. The **×** on a chip removes that value's variant from this product — refused while the variant still holds stock, so goods can never vanish from the books, and other products using the same value are never touched (a value used by nothing anywhere is tidied out of the dropdowns). Adding a value now **creates its variant in the same click** (ticked by default: product's price, zero stock, ready to edit), so a new shade is one action, not two. And to take a value off a single variant without deleting anything, edit the variant and set that dropdown to "—".
