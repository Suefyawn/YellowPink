...
- **Every order now gets an instant confirmation path, even at 3am.** Tonight's order YP-4EZ30H965 exposed a gap: the customer gave no email, staff were asleep, and nothing from us reached them. Three changes close it. The thank-you page now leads with one clear action, a green **"Confirm your order on WhatsApp"** button that pre-types "Confirming my order YP-XXX", so the customer starts the confirmation thread themselves at any hour and staff wake up to a warm chat. The order-confirmation email (COD orders) now tells the customer to expect our WhatsApp message and carries the same confirm-now button, so our staff message never arrives cold from an unknown number. And checkout shows a one-line nudge under the optional email field ("Add your email to get an instant order confirmation, even outside business hours") — the field stays optional, no new friction. About 1 in 5 orders currently arrives without an email; those customers previously heard nothing until staff woke up.
