          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                {['Brand', 'Name', 'Variant', 'Price', 'Category', 'Tag', 'Actions'].map(h => (
            ...
            <tbody>
              {(products as Product[]).map((p, i) => (
                <tr key={p.id} style={{ borderTop: i > 0 ? '1px solid #f3f4f6' : 'none' }}>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: '0.8125rem', color: '#6b7280' }}>{p.variant ?? '—'}</td>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <form action={deleteProduct} onSubmit={e => { if (!confirm(`Delete "${p.name}"?`)) e.preventDefault(); }}>
