...
export interface OnsiteRow { query: string; searches: number; people: number; results: number }
...
export async function getSearchDemand(): Promise<SearchDemand> {
  ...
  if (apiKey) {
    posthog = 'ok';
    try {
      const rows = await phQuery(apiKey, `
        ...
        LIMIT 60`);
      const raw = rows
        .map(r => ({ query: String(r[0] ?? '').trim(), searches: Number(r[1]) || 0, people: Number(r[2]) || 0 }))
      ...
      // term, keeping the chain's peak counts. (Going forward the settle-delay
      // fix in SearchOverlay stops the chains at the source.)
  ...
  return { gscUpdatedAt: gscCache?.updated_at ?? null, winnable, lowCtr, onsite, posthog };
}
