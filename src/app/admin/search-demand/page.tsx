...
function OnsiteTable({ rows }: { rows: OnsiteRow[] }) {
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 520 }}>
      <thead><tr>
...
export default async function SearchDemandPage() {
  ...
  return (
    <div className="adm-page" style={{ padding: '32px 36px', maxWidth: 900 }}>
      ...
      >
        {gaps.length > 0 ? <OnsiteTable rows={gaps} />
      ...
      >
        {winnable.length > 0 ? <GscTable rows={winnable} /> : <Empty>No Search Console data cached yet.</Empty>}
      </Card>
      ...
      {healthy.length > 0 && (
        ...
        >
          <OnsiteTable rows={healthy.slice(0, 20)} />
        </Card>
      )}
