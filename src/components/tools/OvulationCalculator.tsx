...
export function OvulationCalculator() {
  ...
  const rows: { label: string; value: string; strong?: boolean }[] = result
    ? [
        { label: 'Fertile window', value: `${fmt(result.fertileStart)} to ${fmt(result.fertileEnd)}`, strong: true },
        { label: 'Most likely ovulation day', value: fmt(result.ovulation) },
        { label: 'Next period expected', value: fmt(result.nextPeriod) },
        { label: 'Pregnancy test reliable from', value: fmt(result.testFrom) },
      ]
    : [];
    ...
    <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius-card)', padding: 'clamp(20px, 4vw, 32px)' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end' }}>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: '1 1 160px' }}>
          <span className="small-text" style={{ fontWeight: 600 }}>Average cycle length</span>
          <select
            value={cycleLength}
