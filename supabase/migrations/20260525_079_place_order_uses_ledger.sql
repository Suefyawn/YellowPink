-- Migration 079 — route place_order's stock decrement through the
-- inventory ledger.
--
-- Until now place_order ended with an inline UPDATE per item:
--   update public.products set stock = stock - qty where id = pid;
-- This bypassed both the older decrement_stock helper *and* the new
-- record_stock_change ledger-aware RPC introduced in 078, so customer
-- purchases — by far the biggest source of stock movement — never
-- showed up in /admin/inventory. The ledger only carried the import
-- backfill + manual adjustments.
--
-- Fix: replace the inline UPDATE with a record_stock_change call per
-- item. Same atomicity (still one transaction), now with an audit row.
...
begin
  ...
  -- Migration 079: route every line-item stock movement through
  -- record_stock_change so each shows up in /admin/inventory tied to
  -- the order. Same transaction → still atomic with the order INSERT.
