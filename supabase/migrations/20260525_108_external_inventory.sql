-- ============================================================================
-- Per-product "inventory managed externally" flag.
--
-- Some products are fulfilled by a third-party vendor, so Yellow Pink does
-- not hold or count their stock. `track_inventory = false` marks such a
-- product: the storefront treats it as always available, and `place_order`
-- neither validates nor decrements its stock.
--
-- This migration:
--   1. adds the `track_inventory` column (default true — existing products
--      keep their tracked behaviour),
--   2. rewrites `place_order` to skip the stock check + ledger decrement for
--      untracked products,
--   3. rewrites the low-stock notification trigger to ignore untracked
--      products (they should never raise a low-stock alert).
-- ============================================================================
...
comment on column public.products.track_inventory is
  'When false, stock for this product is managed externally (e.g. a third-party '
  'vendor): the storefront treats it as always in stock and place_order does '
  'not validate or decrement its stock.';

-- ── place_order: respect track_inventory ────────────────────────────────────
...
begin
  ...
  -- Route every line-item stock movement through record_stock_change so each
  -- shows up in /admin/inventory tied to the order. Same transaction → still
  -- atomic with the order INSERT. Untracked products are skipped — their
  -- stock is managed by an external vendor, not Yellow Pink.
...
-- ── low-stock trigger: ignore untracked products ────────────────────────────
create or replace function public.notify_low_stock()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if new.track_inventory and new.stock < 5 and (old.stock is null or old.stock >= 5) then
