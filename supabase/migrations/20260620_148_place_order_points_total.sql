-- place_order: subtract redeemed loyalty points from orders.total.
--
-- Pre-existing bug uncovered during the post-launch audit. The checkout UI
-- shows the customer "total = subtotal + shipping - discount - points", but
-- the RPC stored orders.total = subtotal + shipping - discount (line 166 of
-- migration 079) without subtracting points. Consequences if a customer
-- redeems loyalty points:
--   • The COD courier (per /admin/orders/[id] invoice + vendor WhatsApp
--     message) is told to collect the pre-points amount.
--   • Revenue analytics (v_orders_revenue, dashboard_kpis) overstate revenue
--     by the redeemed points value.
--   • The customer pays full cash AND loses points — the opposite of what a
--     redemption should do.
--
-- Forward-only fix: deduct points_redeem from total at insert time and at
-- the client-vs-server total reconciliation. Verified zero historical
-- redemptions in production (loyalty_ledger has no `redeem` rows tied to an
-- order), so no data backfill is required.
--
-- This re-issues place_order with the same signature and behaviour as
-- migration 079 — only the total computation and the client total-check
-- change. The points debit at the bottom of the function is unchanged.
