-- ============================================================================
-- Vendor-scoped free shipping. 2026-07-29.
--
-- Why: a shopper ordered 2 x M-Sol (PKR 2,300 of NB Sons product) and
-- cancelled — the manufacturer's own store (nbsons.com) free-ships above
-- Rs 1,999 while our zone thresholds start at Rs 5,000, so the identical
-- basket was PKR 250 cheaper at the source. Owner approved matching their
-- threshold for their products.
--
-- Mechanism: vendors.free_shipping_threshold (NULL = no vendor rule). When
-- the sum of a basket's line amounts for any single vendor reaches that
-- vendor's threshold, shipping is free for the order. Mirrored in
-- lib/shipping.ts (quote) and here in place_order (the enforcement floor).
-- ============================================================================

alter table public.vendors add column if not exists free_shipping_threshold numeric;
comment on column public.vendors.free_shipping_threshold is
  'PKR: when a basket''s items from this vendor total at least this amount, the order ships free (matches the vendor''s own store, e.g. NB Sons Rs 1,999). NULL = no vendor free-shipping rule.';

-- NB Sons: match nbsons.com's "Free Home Delivery On Orders Above Rs. 1999".
update public.vendors set free_shipping_threshold = 1999
  where id = '81d10e4b-c794-4d3c-977f-e6b862b264fa';
