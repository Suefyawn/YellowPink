-- ============================================================================
-- Free-shipping overcharge clamp. 2026-07-31.
--
-- Why: order YP-ET8YOUQ76 (Rs 3,500 of NB Sons product, well over the
-- vendor's Rs 1,999 free-shipping threshold) was charged Rs 250 delivery.
-- The storefront quote missed the vendor rule (the grid/tile product fetch
-- omitted vendor_id, so tile-added cart lines couldn't qualify), and
-- place_order only rejected UNDERcharges — an overcharge on an order the
-- server itself knew ships free sailed straight through.
--
-- Fix (belt to the storefront's braces): after validating that the client's
-- own arithmetic is coherent, when the server-computed expected rate is 0
-- (vendor rule, zone threshold met, or free-shipping coupon) the stored
-- shipping — and therefore the stored total — is clamped to the free figure.
-- A stale client can still SEND 250; the customer is only ever CHARGED 0.
-- The under-charge floor is unchanged.
--
-- Everything else is identical to migration 770.
-- ============================================================================
