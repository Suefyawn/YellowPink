-- ============================================================================
-- Restore the referral first-order discount in place_order. 2026-07-31.
--
-- Migration 770 (vendor free shipping) rebuilt place_order from a base that
-- predates migration 650, silently dropping the branch that accepts a
-- discount WITHOUT a coupon when a valid referral code is present; 780
-- (overcharge clamp) faithfully copied the same omission. Net effect in
-- production: a referred shopper is shown "10% off your first order" by the
-- checkout UI, but Place Order raises 'discount without coupon'. Found by
-- the 31 July system audit (verified against the deployed function).
--
-- This migration is 780's function with 650's referral branch restored,
-- and nothing else. A regression case now lives in the place_order matrix
-- so the next full rebuild cannot drop the branch unnoticed.
-- ============================================================================
