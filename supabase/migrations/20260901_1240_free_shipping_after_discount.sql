-- ============================================================================
-- Free-delivery threshold applies AFTER the discount. 2026-09-01.
--
-- Owner report: an order whose merchandise subtotal cleared the free-delivery
-- threshold could apply a coupon, drop the amount actually being paid below
-- the threshold, and still ship free — the coupon saved them the discount AND
-- the delivery fee. Free delivery is now earned on what the customer actually
-- pays for the merchandise (subtotal minus the coupon/referral discount), not
-- the pre-discount subtotal.
--
-- Unchanged on purpose:
--   * free-shipping coupons still waive delivery (that IS the coupon's value);
--   * the vendor rule (NB Sons ≥ Rs 1,999) still checks the vendor's own line
--     total — it mirrors the vendor's store and is vendor-funded;
--   * the undercharge floor / overcharge clamp semantics from 780.
--
-- The storefront quote (CheckoutPage → calculateShipping) and the cart's
-- free-delivery progress bar switch to the same discounted figure in the
-- matching code change, so the promise shown always matches the charge.
--
-- Everything below is the 20260731_800 definition with the one threshold
-- change in the shipping-floor section (marked "migration 1240").
-- ============================================================================
...
begin
  ...
  if v_free_ship_coupon
     or v_vendor_free
     or (v_free_enabled and (v_recomputed_sub - v_client_discount) >= v_zone_threshold) then  -- migration 1240: threshold on the discounted subtotal
    v_expected_rate := 0;
  else
